"""NYC taxi zone lookup (LocationID -> borough + zone name).

Generated from taxi_zone_lookup.csv (the TLC zone lookup). Baked in as data
rather than read from a CSV at runtime so the UI is self-contained. IDs 1-265.
Note: a few zone names repeat within a borough (e.g. Queens "Corona"), so the
UI disambiguates display labels by appending the id when a name is not unique.
"""
...
ID_TO_ZONE: dict[int, tuple[str, str]] = {lid: (b, z) for lid, b, z in ZONES}


# borough -> list of (display_label, location_id), sorted by zone name.
# display_label appends the id only when the zone name is not unique in the borough.
def _build_borough_index() -> dict[str, list[tuple[str, int]]]:
    by_borough: dict[str, list[tuple[int, str]]] = {}
    for lid, b, z in ZONES:
        by_borough.setdefault(b, []).append((lid, z))

    index: dict[str, list[tuple[str, int]]] = {}
    for b, entries in by_borough.items():
        name_counts: dict[str, int] = {}
        for _, z in entries:
            name_counts[z] = name_counts.get(z, 0) + 1
        labeled = []
        for lid, z in entries:
            label = z if name_counts[z] == 1 else f"{z} (#{lid})"
            labeled.append((label, lid))
        labeled.sort(key=lambda t: t[0].lower())
        index[b] = labeled
    return index


BOROUGH_INDEX: dict[str, list[tuple[str, int]]] = _build_borough_index()

# Boroughs in a friendly order (real boroughs first, then EWR/edge cases).
_PREFERRED = ["Manhattan", "Brooklyn", "Queens", "Bronx", "Staten Island"]
BOROUGHS: list[str] = [b for b in _PREFERRED if b in BOROUGH_INDEX] + [
    b for b in sorted(BOROUGH_INDEX) if b not in _PREFERRED
]


def label_for_id(location_id: int) -> tuple[str, str, str] | None:
    """Return (borough, display_label, zone_name) for a location id, or None."""
    hit = ID_TO_ZONE.get(location_id)
    if hit is None:
        return None
    borough, zone = hit
    for label, lid in BOROUGH_INDEX[borough]:
        if lid == location_id:
            return borough, label, zone
    return borough, zone, zone
